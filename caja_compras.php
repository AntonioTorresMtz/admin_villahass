    ...
    <div class="creditos__tabla">
        ...
        <div class="tabla_creditos">
            ...
            <div class="tabla creditos">
                    ...
                    c.fecha_limite, c.monto_deuda, c.estado FROM credito_compra AS c
                    INNER JOIN compras AS v
                    ...
                    while($row = mysqli_fetch_assoc($resultado)) {?>
                        ...
                        <?php  $dinero = $row['monto_deuda'];
                            $final = formatoDinero($dinero); ?>

                            <div class="table_item"> <?php echo $row["id_creditoCom"] ?> </div>
...
<div class="menu_caja1">
    ...
    <div class="ventas__menu">
        <div class="tabla_pagos">
            ...
            <div class="tabla pagos">
                <?php 
                $ventas = "SELECT a.id_pagoCom, b.nombre, a.fecha, a.monto, a.metodo, a.id_caja, c.id_compra 
                FROM pagos_compras a INNER JOIN caja b 
                on a.id_caja = b.id_caja 
                INNER JOIN compras c 
                ON c.id_pago = a.id_pagoCom; ";
                $resultado = mysqli_query($conn, $ventas);
